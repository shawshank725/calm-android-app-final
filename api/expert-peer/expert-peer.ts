...
export const useAllExpertPeerSlots = () => {
    return useQuery<ExpertSlot[]>({
        ...
        queryFn: async () => {
        const {data, error} = await supabase.from("expert_slots").select("*").order("start_time", {ascending: true});
        if (error) {
            throw new Error(error.message);
        }
        return data;
...
export const useDeleteSlot = () => {
    return useMutation({
        async mutationFn(data:ExpertSlot) {
            const result = await supabase.from('expert_slots').delete().match({"start_time": data.start_time});
            if (result.error) {
                throw new Error(result.error.message);
            }
            return result.data;
